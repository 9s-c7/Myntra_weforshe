...
function InfiniteScroll() {
    ...
    const imageUrl = items.length > 0 ? `http://localhost:3000${items[currentIndex].imageUrl}` : null;
    ...
    const fetchItems = async () => {
        try {
            const response = await axios.get('http://localhost:3000/items?approval=1');
    ...
    const fetchComments = async (itemId) => {
        try {
            const response = await axios.get(`http://localhost:3000/items/${itemId}/comments`);
    ...
    const addComment = async () => {
        ...
        try {
            const response = await axios.post(`http://localhost:3000/items/${items[currentIndex]._id}/comments`, { comment: newComment });
    ...
    return (
        <ImageBackground style={styles.screen}>
            ...
            <Feature style={styles.feature} triggerTintEffect={triggerTintEffect} />
            {isModalVisible && (
                <CommentSection
                    comments={comments}
